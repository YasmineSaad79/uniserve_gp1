...
class _DoctorProfileScreenState extends State<DoctorProfileScreen> {
  ...
  Future<void> fetchProfile() async {
    ...
    // تأكد من استخدام الـ IP الصحيح (10.0.2.2 هو لـ Android Emulator)
    final url =
        Uri.parse('http://10.0.2.2:5000/api/doctor/profile/${widget.doctorId}');
    try {
  ...
  // ... (بقية دوال updateProfile, changePassword, uploadPhoto, pickImage)

  // ملاحظة: دوال updateProfile و changePassword و uploadPhoto هي نفسها
  // كما في كودك الأصلي، وهي تعمل بشكل صحيح الآن بعد تعديل الـ Backend.
  ...
  Future<void> changePassword() async {
    ...
    setState(() => isUpdating = true);
    final url = Uri.parse(
        'http://10.0.2.2:5000/api/doctor/profile/password/${widget.doctorId}');
    try {
      final response = await http.put(
        url,
        headers: {'Content-Type': 'application/json'},
  ...
  Future<void> uploadPhoto() async {
    ...
    final url = Uri.parse(
        'http://10.0.2.2:5000/api/doctor/profile/photo/${widget.doctorId}');
    final request = http.MultipartRequest('PUT', url);
    // التأكد من أن اسم الحقل هنا هو 'photo' ليطابق ما ينتظره الـ Backend
    request.files
  ...
  Widget build(BuildContext context) {
    ...
    // 1. ملف جديد تم اختياره (_imageFile)
    // 2. URL صورة سابقة موجودة (currentPhotoUrl)
    final ImageProvider<Object>? profileImage = _imageFile != null
        ? FileImage(_imageFile!)
        : (currentPhotoUrl != null && currentPhotoUrl != ""
            ? NetworkImage('http://10.0.2.2:5000$currentPhotoUrl')
